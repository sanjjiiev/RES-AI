...
RES-AI is a 5-phase AI pipeline that turns raw resume PDFs into a queryable **knowledge graph** in Neo4j, then ranks candidates against a job description document (DOCX/PDF) using **graph traversal**, weighted scoring, and an optional **local LLM** (Ollama).
...
| Black-box AI decisions | Every score is traceable to graph edges with matched skills and identified gaps |
...
└─────────────────────────────────────────────────────────┘
    ...
    ▼
┌─────────────────────────────────────────────────────────┐
│ PHASE 4 — JD Evaluation  (jd_parser.py & evaluator.py) │
│                                                         │
│  Docling + GLiNER parse JD DOCX/PDF into JDProfile     │
│  (extracts required/nice-to-have skills, certs, degrees)│
│                                                         │
│  Neo4j graph ranks candidates by weighted skill coverage│
...
├── jd_parser.py       # Phase 4: JD DOCX/PDF parser → JDProfile
├── evaluator.py       # Phase 4: weighted candidate scorer
...
Instead of a plain text JD, you provide a JD document (DOCX/PDF) containing company info, role description, and requirements. The system parses it to extract key skills and rank candidates based on a weighted scoring model.

```powershell
# Pass a JD DOCX/PDF file
python run.py evaluate --jd-file "path\to\JobDescription.docx"

# With Ollama LLM narrative summaries (requires Ollama running)
python run.py evaluate --jd-file "JD.docx" --llm --top-n 5
```

**Sample output:**
```
JD PARSED: JD: Senior Python Developer | 6 required skills | 3 nice-to-have | exp≥3yr
  Required skills   : Python, Fastapi, Postgresql, Docker, Aws, Redis
  Nice-to-have      : Kubernetes, Terraform
  Certifications    : Aws Certified Developer
  Min experience    : 3 yr
────────────────────────────────────────────────────
CANDIDATE RANKINGS  (12 evaluated)
====================================================
Rank  1 | CANDIDATE_A1B2C3D4E5F6  | score=14 | coverage=83%
         matched : Python, Fastapi, Postgresql, Docker, Aws
         gaps    : Redis
Rank  2 | CANDIDATE_G7H8I9J0K1L2  | score=8  | coverage=50%
         matched : Python, Aws, Postgresql
         gaps    : Fastapi, Docker, Redis
```

**Scoring Logic:**
- **Required skill:** +2 points
- **Nice-to-have skill:** +1 point
- **Certification:** +3 points
- **Degree:** +2 points
- **Meets minimum experience:** +2 points
...
| `POST` | `/evaluate` | Upload a JD DOCX/PDF to evaluate candidates |
